...
const AddHotel = () => {
  ...
  return (
    <form className='mt-4 p-4'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 my-4'>
        {Object.keys(image).map((key) => (
          <label htmlFor={`hotelImage${key}`} key={key}>
            <img
              className='opacity-80 cursor-pointer max-h-13'
              src={image[key] ? URL.createObjectURL(image[key]) : uploadArea}
            />
            <input id={`hotelImage${key}`} type='file' accept='image/*' hidden onChange={(e) => setImage({ ...image, [key]: e.target.files[0] })} />
          </label>
        ))}
      </div>
      <div className='w-full flex sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-50'>
          ...
          <select value={inputData.roomType} onChange={(e) => setInputData({ ...inputData, roomType: e.target.value })}
            className='border opacity-70 border-gray-300 mt-1 rounded w-full p-2'
          >
      ...
      <div className='flex flex-col text-gray-400 mt-2  flex-wrap'>
      ...
      <button className='text-white bg-amber-500 rounded p-2 mt-4 transition-colors hover:bg-amber-600'>
        Add Hotel
      </button>
    </form>
  )
